...
void drawFlag()
{
    float stripeH = 600.0f / 13.0f;

    for (int i = 0; i < 13; i++)
    {
        if (i % 2 == 0) setColor(0.70f, 0.0f, 0.0f);
        else setColor(1.0f, 1.0f, 1.0f);
        drawRectangle(0, 600 - (i + 1) * stripeH, 900, 600 - i * stripeH);
    }

    float cantonW = 380.0f;
    float cantonH = stripeH * 7.0f;
    setColor(0.0f, 0.0f, 0.35f);
    drawRectangle(0, 600 - cantonH, cantonW, 600);

    setColor(1.0f, 1.0f, 1.0f);
    float rowGap = cantonH / 10.0f;
    float colGap = cantonW / 7.0f;

    for (int row = 0; row < 9; row++)
    {
        int starsInRow = (row % 2 == 0) ? 6 : 5;
        float y = 600 - rowGap * (row + 1);
        float startX = (row % 2 == 0) ? colGap * 0.65f : colGap * 1.10f;

        for (int col = 0; col < starsInRow; col++)
        {
            float x = startX + col * colGap;
            drawFilledStar(5, x, y, 10, 4.2f, 90);
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("USA Flag - CG Lab Assignment 1");
    init();
